...
export function DispatchesScreen({ navigation }: any) {
    const { data, isLoading, refetch, isRefetching } = useDispatches();
    ...
    const renderItem = ({ item }: any) => (
        <View style={styles.card}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
                <Text style={styles.title}>Challan: {item.challanNumber || item.reference}</Text>
                <Text style={styles.status}>{item.status}</Text>
            </View>
            <Text style={styles.subtext}>Date: {new Date(item.date).toLocaleDateString()}</Text>
            <Text style={styles.subtext}>{item.items?.length || 0} items moved</Text>
        </View>
    );

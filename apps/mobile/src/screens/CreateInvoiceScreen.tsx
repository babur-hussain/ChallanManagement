...
import { useCreateInvoice, useNextInvoiceNumber, useInvoicePreview } from '../hooks/api/useInvoices';
...
export function CreateInvoiceScreen({ navigation }: any) {
  ...
  const [invoiceType, setInvoiceType] = useState('SUPPLY_ON_APPROVAL');
  ...
  const invoiceCfg = (settingsData as any)?.invoices || {};
  const showRates = invoiceCfg.showRates ?? true;
  const showAmount = invoiceCfg.showAmount ?? true;

  // Pre-fill defaults from settings
  const [defaultsLoaded, setDefaultsLoaded] = useState(false);
  useEffect(() => {
    if (settingsData && !defaultsLoaded) {
      const cfg = (settingsData as any)?.invoices;
  ...
  const buildPayload = (isDraft = false) => {
    ...
    return {
      ...
      dueDate: invoiceDate, // we can support custom due date UI later, defaulting to invoiceDate
      ...
      items: validItems.map((li) => ({
        ...
        gstRate: li.taxRate || 5, // use taxRate for gstRate
      })),
      adjustment: parseFloat(adjustmentAmount) || 0,
      notes: customerNotes.trim() || undefined,
      termsAndConditions: termsAndConditions.trim() || undefined,
      isDraft,
      shippingCharges: 0, // add a UI for this later if needed
    };
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!netState.isConnected) {
      const offlineInvoice = await OfflineSyncService.enqueueInvoiceCreation(payload as any);
      Alert.alert('📴 Saved Offline', `Invoice queued as ${offlineInvoice._localId}. It will sync when you're back online.`);
    ...
    createMutation.mutate(payload as any, {
      onSuccess: (invoice: any) => {
        Alert.alert('✅ Success', `Invoice ${invoice.invoiceNumber} created!`, [
          { text: 'View', onPress: () => navigation.replace('InvoiceDetail', { id: invoice._id }) },
          { text: 'OK', onPress: () => navigation.goBack() },
        ]);
      },
      onError: (err: any) => {
        Alert.alert('Error', err?.response?.data?.message || err.message || 'Failed to create invoice');
      }
    });
  };
  ...
  const handleSaveDraft = async () => {
    ...
    createMutation.mutate(payload as any, {
      onSuccess: (invoice: any) => {
        Alert.alert('✅ Draft Saved', `Invoice ${invoice.invoiceNumber} saved.`);
        navigation.goBack();
      },
      onError: (err: any) => {
  ...
  const previewPayload = useMemo(() => {
    ...
    return {
      invoiceNumber: nextNumber?.nextStr || 'INV-PREVIEW',
      invoiceDate: invoiceDate || new Date().toISOString(),
      dueDate: invoiceDate || new Date().toISOString(),
      ...
      items: validItems.map((item: any) => ({
        ...
        quantity: item.totalMeters || 0,
        ratePerUnit: item.ratePerMeter || 0,
  ...
  }, [showPreview, nextNumber, lineItems, selectedParty, referenceNumber, customerNotes, termsAndConditions, adjustmentAmount, invoiceDate]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          <Text style={{ fontSize: 11, color: colors.textSecondary, marginBottom: 2 }}>Invoice Type</Text>
          <View style={{ flexDirection: 'row', gap: 4, marginBottom: spacing.sm }}>
            {[{ v: 'JOB_WORK', l: 'Job Work' }, { v: 'SUPPLY_ON_APPROVAL', l: 'Supply on Approval' }, { v: 'OTHERS', l: 'Others' }].map(t => (
              <TouchableOpacity key={t.v} style={[styles.segBtn, invoiceType === t.v && styles.segBtnActive]} onPress={() => setInvoiceType(t.v)}>
                <Text style={[styles.segBtnText, invoiceType === t.v && styles.segBtnTextActive]}>{t.l}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <TextInput style={[styles.input, { height: 60, textAlignVertical: 'top' }]} placeholder="Notes to display on invoice" placeholderTextColor={colors.textMuted} value={customerNotes} onChangeText={setCustomerNotes} multiline />
        ...
        <View style={{ flexDirection: 'row', gap: spacing.sm, margin: spacing.lg }}>
          ...
          <TouchableOpacity style={[styles.submitBtn, { flex: 2 }, (createMutation.isPending || !selectedParty || lineItems.length === 0) && styles.disabledBtn]} onPress={handleSubmit} disabled={createMutation.isPending || !selectedParty || lineItems.length === 0}>
            {createMutation.isPending ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitBtnText}>Create Invoice</Text>}
          </TouchableOpacity>
        </View>

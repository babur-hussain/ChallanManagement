...
import { useAttributes, useCreateAttribute, useUpdateAttribute, useDeleteAttribute } from '@/hooks/api/useAttributes';
import { createAttributeSchema } from '@textilepro/shared';

// For attributes processing strings to array
const processForm = (data: any) => {
  if(typeof data.options === 'string') data.options = data.options.split(',').map((o: string) => o.trim());
  return data;
};
...
export function AttributesManager() {
  ...
  const { data, isLoading } = useAttributes({ page, limit: 20, search });
  ...
  const handleEdit = (item: any) => {
    ...
    if(Array.isArray(item.options)) populated.options = item.options.join(', ');
    reset(populated);
    setIsModalOpen(true);
  };
  ...
  const columns: Column<any>[] = [
      ...
      { key: 'options', header: 'Options (Comma separated)', cell: (item: any) => <span>{item.options?.join(', ')}</span> },
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center bg-card p-4 rounded-md border">
        ...
        <Button onClick={handleAddNew}><Plus className="mr-2 h-4 w-4" /> Add Attributes</Button>
      </div>
      ...
      <FormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Attribute' : 'Add Attribute'} isLoading={createMut.isPending || updateMut.isPending}>
      ...
      <div className="space-y-1.5">
        ...
        <Input id="options" placeholder="e.g. Red, Blue, Green" {...register('options')} />
        {errors.options && <p className="text-xs text-destructive">{String(errors.options.message)}</p>}
      </div>

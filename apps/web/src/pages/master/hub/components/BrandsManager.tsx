...
import { useBrands, useCreateBrand, useUpdateBrand, useDeleteBrand } from '@/hooks/api/useBrands';
import { createBrandSchema } from '@textilepro/shared';
...
export function BrandsManager() {
  ...
  const { data, isLoading } = useBrands({ page, limit: 20, search });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center bg-card p-4 rounded-md border">
        ...
        <Button onClick={handleAddNew}><Plus className="mr-2 h-4 w-4" /> Add Brands</Button>
      </div>
      ...
      <FormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Brand' : 'Add Brand'} isLoading={createMut.isPending || updateMut.isPending}>
